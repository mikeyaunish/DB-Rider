REBOL [
	...
	History: {
	    - MIKE YAUNISH: 
	        Modified my-to-date function to force use of YYYY-MM-DD date format
	        Remove "to decimal!" in conv-model to support leaving decimal point data alone. 
	        So $'23.00' is pulled in correctly without converting it to '23.0'
...
;------ Internals --------
    ...
    num-pad: func [text length [integer!] /with padding [char!]][
	...
	my-to-date: func [v][ 
	    either attempt [rdate: to date! v ] [
	        to-YYYY-MM-DD rdate
	    ][
	        "0000-00-00"
	    ]
    ...
    my-to-datetime: func [v /local p ][   
	    any [attempt [
	        v: to-date v 
	        rejoin [ pad v/year 4 "-" pad v/month 2 "-" pad v/day 2 " " v/time ]
	    ] "0000-00-00 00:00:00" ]
	]
	
	conv-model: [
		;decimal			[to decimal!]
		decimal			none ; to accomodate decimal points correctly 
		tiny			[to integer!]
		short			[to integer!]
		long			[to integer!]
		float			[to decimal!]
		...
		new-decimal		none ; to preserve decimal points 
		enum			none
		set				none
		tiny-blob		none
		medium-blob		none
	...
	insert-all-queries: func [port [port!] data [string!] /local s e res d][
		...
		parse/all s: data [
			any [
				...
				|{"} any [{\"} | {""} | not-dquote] {"}
				| #"`" thru #"`"
				| "begin" thru "end" ;" to match mismatched quote for syntax highlighting.
				| e: d (
					clear sql-buffer
